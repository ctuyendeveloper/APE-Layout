...
export function Screen2() {
    ...
    const [quantityInputs, setQuantityInputs] = useState(Array(Dataproduct.length).fill({ value: "", error: "" }));
    ...
    const handleChangeQuantity = (value: string, index: number) => {
        ...
        if (quantityValue > parseInt(Dataproduct[index].tonkho)) {
    ...
    const renderItem = (item: { id: any; name: any; tonkho: any; mucantoan: any; }, index: number) => {
        ...
        return (
            <View style={styles.item} key={index}>
                <TouchableOpacity style={styles.itemmain} onPress={() => setSelectedItemIndex(index)}>
                    ...
                    <Text style={styles.txtten}>{item.id}. {item.name}</Text>
                    <View style={styles.tonkho}>
                        <Text style={styles.txttonkho}>Tồn kho: <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#575E69' }}>{item.tonkho}</Text></Text>
                        <Text style={styles.txtmucantoan}>Mức an toàn: <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#575E69' }}>{item.mucantoan}</Text></Text>
                    </View>
                </TouchableOpacity>
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.view2}>
                ...
                <Text style={styles.texttotal}>Tổng cộng (món): <Text style={{ fontWeight: 'bold', color: '#1F2937', fontStyle: 'italic' }}>{Dataproduct.length}</Text></Text>
                <View style={styles.view4}>
                    ...
                    <ScrollView contentContainerStyle={styles.scrollViewContent}>
                        {Dataproduct.map((item, index) => renderItem(item, index))}
                    </ScrollView>
                </View>
            </View>
